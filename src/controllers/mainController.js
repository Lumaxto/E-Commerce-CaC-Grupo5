...
module.exports = {
  getListadoTienda: async (req, res) => {
    try {
      const [registros] = await conn.query(`
        SELECT productos.*, categorias.categoria AS categoria_categorias 
        FROM productos 
  ...
  crearRegistro: async (req, res) => {
    const sql = `INSERT INTO productos (nombre, origen, precio, imagen, categoria_id) VALUES (?, ?, ?, ?, ?)`;
    try {
  ...
  actualizar: async (req, res) => {
    const sql = `UPDATE productos SET nombre=?, origen=?, precio=?, imagen=?, categoria_id=? WHERE id=?`;
    const { idMod, nombre, origen, precio, imagen, categoria_id } = req.body;
    try {
